    ...
    def generate(self, prompt, temperature=0.5, provider=None, model=None, api_keys=None):
        ...
        if not model:
            if active_provider == self.default_provider and self.default_model:
                ...
                from configuration.backend_config.config import MODEL_GOOGLE
                model = MODEL_GOOGLE
            elif active_provider == "openrouter":
                from configuration.backend_config.config import MODEL_OPENROUTER
                model = MODEL_OPENROUTER
            elif active_provider == "nvidia":
                from configuration.backend_config.config import MODEL_NVIDIA
                model = MODEL_NVIDIA
